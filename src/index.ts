...
//     const browser = await puppeteer.launch({executablePath: process.env.TWITCH_CHROME_EXECUTABLE, args:['--no-sandbox']});
...
async function run() {
    ...
    for (var i = 0; i < streamers.length; i++) {
        streamersBrowser[i] = await puppeteer.launch({executablePath: process.env.TWITCH_CHROME_EXECUTABLE, args:['--no-sandbox']});
